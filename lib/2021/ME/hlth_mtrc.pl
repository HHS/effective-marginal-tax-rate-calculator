#=============================================================================#
#  Public Health Insurance -- ME 2021 
#=============================================================================#
#	
...
sub hlth
{
    ...
    our $a27yo_premium = (0, 410, 569, 371, 410, 569, 391, 391, 391, 391, 473, 473, 371, 473, 410, 569, 371)[$in->{'ccdf_region'}];  # This is the monthly premium for the second-lowest cost silver plan (SLCSP) for 27-year-olds in Maine for all counties, rounded. It's actually 331.79. Attained from https://www.healthcare.gov/health-and-dental-plan-datasets-for-researchers-and-issuers/. Updated in 2021. The CCDF region is from the FRS_Locations table, and the input will be pulled into this. 
    our $a27yo_premium_ratio = 1.048;  # This is the federally-mandated premium ratio for 27-year-olds. Maine does not have a state-specific age curve. As of 2021, this has not changed since 2016. Available at https://www.cms.gov/CCIIO/Resources/Regulations-and-Guidance/Downloads/Final-Guidance-Regarding-Age-Curves-and-State-Reporting-12-16-16.pdf. 

	our $child_1to18_medicaid_limit = 1.61;    # child income limit as % of FPG for children ages 1 to 5. This is 215% MAGI Medicaid income limit plus teh 5% MAGI disregard.
	our $child_age0_medicaid_limit = 1.96;    # child income limit as % of FPG for children less than 1 and pregnant women. This is 215% MAGI Medicaid income limit plus teh 5% MAGI disregard.
	our $parent_19to20_medicaid_limit = 1.6;  # adult income limit as % of FPG. It's 133% plus the 5% MAGI disregard.
	our $parent_medicaid_limit = 1.38;  # adult income limit as % of FPG. It's 133% plus the 5% MAGI disregard.
	our $child_chip_limit = 2.13;    # child income limit as % of FPG. This is the limit on CHIP, below which there are premiums.
	our $mwd_medicaid_limit = 2.5; #The limit for eligibility for earned plus unearned income for participants in Maine's Workng While Disabled program. 
	our $mwd_medicaid_unearned_limit = 1; #The limit for eligibility for unearned income participants in Maine's Workng While Disabled program. 
	our $mwd_single_income_limit = 2684; # The MWD income limit at 250% of the federal poverty level in 2021, but specifically as published in the MA handbook. See http://services.dpw.state.pa.us/oimpolicymanuals/ma/316_MAWD/316_Appendix_A.htm. Pennsylvania has a similar policy as Maine and the same income limits. It seems logical and likely federal policy to use apply the same method here. 
	our $mwd_married_income_limit = 3630;
	# outputs created
    ...
    our $hlth_cov_parent = 'NA';   #@ health insurance status of parents (Medicaid, employer, nongroup, user-entered). It is a concatenated variable.
	our $adult_medicaid_count = 0; #How many adults in the hh are on Medicaid.
	our $adult_chip_count = 0;
	our $adult_mwd_count = 0;
	our $hlth_cov_child_all = 'NA';  # health insurance status of all children (Medicaid, employer, individual).
	...
	our $parent1_mwd = 0; #Helpful for assigning MWD premiums.						
	our $parent2_mwd = 0; #Helpful for assigning MWD premiums.						
	our $parent3_mwd = 0; #Helpful for assigning MWD premiums.						
	our $parent4_mwd = 0; #Helpful for assigning MWD premiums.						
	...
	if ($in->{'hlth'} == 0 && $in->{'premium_tax_credit'} == 0) { #No Medicaid, no PTC. This doesn't happen in the MTRC since we always set premium_tax_credit to 1. So not worrying too much about this if-block for now.
		...
		#
		# We now apply MAGI income limits and disregards based on adult and child limits. There may be separate eligibility criteria for non-MAGI Medicaid options (which seem to be rare across states) but we are not including them in the MTRC for now. 
		...
		if ($in->{'hlth'}== 1) { 
			...
			for(my $i=1; $i<=4; $i++) {
				if ($in->{'parent'.$i.'_age'} > -1) {
					if ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= $parent_medicaid_limit) {
					...
					} elsif ($in->{'parent'.$i.'_age'}<=20 && $out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= $parent_19to20_medicaid_limit) {  
						${'hlth_cov_parent'.$i} = 'Medicaid';
					} elsif ($in->{'parent'.$i.'_age'}==18 && $out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= $child_1to18_medicaid_limit) {  
						${'hlth_cov_parent'.$i} = 'Medicaid';
						...
						${'hlth_cov_parent'.$i} = 'Medicaid';
					} elsif ($in->{'disability_parent'.$i} == 1 && $out->{'parent'.$i.'_earnings'} > 0 && (($out->{'parent'.$i.'ssi_married'} == 0 && $out->{'parent'.$i.'_ssi_income'}  <= $mwd_single_income_limit && $out->{'parent'.$i.'_ssi_unearned_income'} < ($mwd_medicaid_unearned_limit / $mwd_medicaid_limit) * $mwd_single_income_limit) || ($out->{'parent'.$i.'ssi_married'} == 1 && $out->{'parent'.$i.'_ssi_income'} <= $mwd_married_income_limit && $out->{'parent'.$i.'_ssi_unearned_income'} < ($mwd_medicaid_unearned_limit / $mwd_medicaid_limit) * $mwd_married_income_limit))) { 
						#These are the non-resource eligibility criteria the mwd program in Pennsylvania. Other states have similar programs. We are definiing disability as former receipt of SSI or SSDI, which is a qualifying condition for mwd Medicaid. As we are not asking about assets/resources in the MTRC, we are excluding those criteria for this program for now. THe program has a $10,000 asset test. This covers the "Worker with a Disability" category in PA; we are not considering (for now) the "Worker with a Medically Improved Disability." The distinction between the two is largely whether the person formerly has received SSI/SSDI or not, with the latter being non-beneficiaries, and therefore there is a work requirement attached to that category (40 hours/month). We are defining disability in the MTRC as whether someone received SSI/SSDI int the past, so only the "Worker with a disabiltiy" category is relevant here. 
						#Note that for mwd, the unit is composed of only the SSI eligible individual and their spouse if they have one. Children are not counted.
						${'hlth_cov_parent'.$i} = 'Medicaid';
						${'parent'.$i.'_mwd'} = 1; #Helpful for assigning mwd premiums.						
					...
					if (${'hlth_cov_parent'.$i} eq 'CHIP') {
						...
						#Adding in the MAGI disregard of 5% to levels in Chart 8 of the Maine Medicaid manual
						#Because it's easier to do this up here rather than down below.
						if ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} < 1.571 +.05 || $adult_chip_count >= 2) {
							#Monthly CHIP premiums are added up to to two children.
							$adult_medicaidchip_premiums += 0;	
						} elsif ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= 1.66 + .05) {
							$adult_medicaidchip_premiums += 8*12;	
						} elsif ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= 1.77 + .05) {
							$adult_medicaidchip_premiums += 16*12;	
						} elsif ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= 1.92 + .05) {
							$adult_medicaidchip_premiums += 24*12;
						} elsif ($out->{'percent_of_poverty_parent'.$i.'_medicaid'} <= 2.08 + .05) {
							$adult_medicaidchip_premiums += 32*12;
						}
					}
					#"If countable monthly income is over 150% and equal to or less than 200% of the FPL, the monthly premium is $10.00. If countable monthly income is over 200% and less than 250% of FPL the monthly premium is $20.00." - Maine Medicaid eligibilty manual
					#"If a couple is eligible under this coverage group, there is one premium for the couple based on the couple’s countable income." - Maine Medicaid eligibilty manual
					#The above means that there is, at most, one premium based on MWD eligibility, even if both adults in a couple are receiving Medicaid through MWD.
					if (${'parent'.$i.'_mwd'} == 1 && $adult_mwd_count == 0) { #Participation in the mwd programs includes a premium, but those are largely waived durig the COVID pandemic. Technically they are only waived for individuals enrolling in mwd after the pandemic hit, but to reduce question burden, we are not asking when, exactly, people enrolled in certain types of Medicaid programs.
						$adult_mwd_count += 1;
						if ($out->{'parent'.$i.'_ssi_income'} <= 1.5) {	
							$adult_medicaidchip_premiums += 0;
						} elsif  ($out->{'parent'.$i.'_ssi_income'} <= 2) {
							$adult_medicaidchip_premiums += 10 * 12;
						} else { # $out->{'parent'.$i.'_ssi_income'} <= 2.5, the limit.
							$adult_medicaidchip_premiums += 20 * 12;
						}
					}
				}
			} 
			...
			#
			# CHILDREN'S COVERAGE  	
			# Note that in Maine, there are several potential Medicaid limits for children. We need to decouple this coverage from CHIP because CHIP has premiums and some programs -- like WIC -- may rely on Medicaid eligibility as categorical but not CHIP.
			for(my $i=1; $i<=$in->{'child_number'}; $i++) {
				if (($in->{'child'.$i.'_age'} == 0 && $out->{'percent_of_poverty_children_medicaid'} <= $child_age0_medicaid_limit) || ($in->{'child'.$i.'_age'} >= 1 && $in->{'child'.$i.'_age'} <= 18 && $out->{'percent_of_poverty_children_medicaid'} <= $child_1to18_medicaid_limit)) { 
					...
					if (${'hlth_cov_parent'.$i} eq 'CHIP') {
						...
						#Adding in the MAGI disregard of 5% to levels in Chart 8 of the Maine Medicaid manual
						#Because it's easier to do this up here rather than down below.
						if ($out->{'percent_of_poverty_children_medicaid'} < 1.571 +.05 || $adult_chip_count + $child_chip_count >= 2) {
							#Monthly CHIP premiums are added up to to two children.
							$chip_premiums += 0;	
						} elsif ($out->{'percent_of_poverty_children_medicaid'} <= 1.66 + .05) {
							$chip_premiums += 8*12;	
						} elsif ($out->{'percent_of_poverty_children_medicaid'} <= 1.77 + .05) {
							$chip_premiums += 16*12;	
						} elsif ($out->{'percent_of_poverty_children_medicaid'} <= 1.92 + .05) {
							$chip_premiums += 24*12;
						} elsif ($out->{'percent_of_poverty_children_medicaid'} <= 2.08 + .05) {
							$chip_premiums += 32*12;
						}
					}
				}
	...
	if ($in->{'privateplan_type'} eq 'individual' || ($in->{'privateplan_type'} eq 'employer' && $in->{'hlth_plan_estimate_source'} ne 'user-entered')) { 
		...
		#The above costs include reductions from ACA subsidies. We also gather  the total unsubsidized costs for the household, so we can compare it against user overrides and estimate the value of the premium tax credit the family receives.
	...
	if ($hlth_cov_parent eq 'Medicaid') {
		$health_expenses = 0 + $adult_medicaidchip_premiums; 
    ...
    # Medically Needy programs, "spend down" programs, or "In and Out Medical Assistance," are also programs that can be added. But for now, this program just covers MAGI Medicaid and Medicaid for SSI-related groups.
	
	#Note that there is no asset test for Medicaid programs. Some states set asset limits as a requirement to participate in Medically Needy programs.
	...
	#debugging
	foreach my $debug (qw(hlth_cov_parent health_expenses parent_cost family_cost family_cost_individual_total child_number parentswitch_dummy familyswitch_dummy adult_medicaidchip_premiums child_medicaid_count hlth_cov_child_all chip_premiums premium_credit_recd)) {
		print $debug.": ".${$debug}."\n";
	}
