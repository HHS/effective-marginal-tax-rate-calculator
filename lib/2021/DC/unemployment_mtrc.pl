...
sub unemployment {
	...
	#our $fulltime_hours = 35; #The amount of hours under which an individual is never considered working full time, meaning that they will still be considered to be working part time. This is a policy variable for some states (e.g. Pennsylvania), but not in DC.
	...
	our $ui_earnings_disregard_amt = 50; # #Earnings disregard for computing weekly benefit amount is $51 + 1/3 wages. This amount is  disregarded from weekly earnings when individual is working and eligible for partial unemployment. 
	our $ui_earnings_disregard_pct = 1/3; # #Earnings disregard for computing weekly benefit amount is $51 + 1/3 wages
	our $ui_excess_earnings_disregard = 20; #The amount above the WBA for which a person on unemployment is still eligible for part-time unemployemnt
	our $ui_dependency_allowance_perchild = 0; #DC has no dependency allowance (simplifying this code). 
	our $ui_dependency_allowance_max = 0; 
	our $max_wba = 432; 
	...
	if ($in->{'ui'} == 1) {
		...
		for(my $i=1; $i<=$in->{'family_structure'}; $i++) {
			if ($in->{'parent'.$i.'_ui_recd_initial'} == 0) { 
				...
				${'parent'.$i.'_wba_after_disregard'} = &pos_sub($in->{'parent'.$i.'_ui_recd_initial'}, $fpuc_recd_initial) + &pos_sub($out->{'parent'.$i.'_earnings_initial'} / 52, $ui_excess_earnings_disregard); #We  remove any FPUC payments the adult is receiving. We'll add that back in if the adult is selecting to model contintinual COVID UI expansions.
				#We also disregard the first $20 in earnings, 
				...
				if ($out->{'parent'.$i.'_earnings_initial'} / 52 <= $ui_earnings_disregard_amt) { #If they don't make more than the disregard, all earnings are disregarded in the computation of benefits received:
					${'parent'.$i.'_wba'} = &least($max_wba, &pos_sub($in->{'parent'.$i.'_ui_recd_initial'}, $fpuc_recd_initial));
				} else {
					${'parent'.$i.'_wba'} = &least($max_wba, &pos_sub($in->{'parent'.$i.'_ui_recd_initial'}, $fpuc_recd_initial) + ($out->{'parent'.$i.'_earnings_initial'} / 52 - ($ui_earnings_disregard_amt + $ui_earnings_disregard_pct * $out->{'parent'.$i.'_earnings_initial'} / 52))); 
				}
				#The above is mathematically derived. Can provide details upon request.
				
				#${'parent'.$i.'_wba'} = &least($max_wba, ${'parent'.$i.'_wba_after_disregard'} + $ui_earnings_disregard_amt + $ui_earnings_disregard_pct * $out->{'parent'.$i.'_earnings_initial'} / 52); 
				...
				if ($out->{'parent'.$i.'_earnings'} / 52 > $ui_excess_earnings_disregard + ${'parent'.$i.'_wba'}) {
					...
					${'parent'.$i.'_ui_recd_wk'} = &pos_sub(${'parent'.$i.'_wba'},&pos_sub($out->{'parent'.$i.'_earnings'} / 52, $ui_earnings_disregard_amt + $ui_earnings_disregard_pct * $out->{'parent'.$i.'_earnings'} / 52)); #Earnings are subtracted less the earnings disregard, which varies by state.
