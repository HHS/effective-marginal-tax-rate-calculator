...
sub defaults {
    ...
    @{$self->{'order'}} = qw(interest parent_earnings filing_status unemployment transportation ssp ssi fed_hlth_insurance hlth child_support child_care tanf child_care tanf ccdf tanf ccdf sec8 fsp_assets liheap fsp schoolsummermeals wic fedtax eitc payroll ctc statetax food lifeline salestax other);
			
	
	#Below is an explanation of the "order" above, specific to New Hampshire. Please note that there are some repeated codes/functions, also explained below. 
