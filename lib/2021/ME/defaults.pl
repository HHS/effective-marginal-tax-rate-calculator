...
sub defaults {
    ...
    @{$self->{'order'}} = qw(interest parent_earnings filing_status unemployment transportation ssp ssi fed_hlth_insurance hlth child_support child_care ccdf tanf child_care ccdf tanf sec8 tanf sec8 fsp_assets liheap fsp schoolsummermeals wic fedtax eitc payroll ctc statetax food lifeline salestax other);
	
		
	#Below is an explanation of the "order" above, specific to New Hampshire. This is provided for illustrative purposes, to explain some methodological uses for NH. Other states have different loops based on program rules. 
