#=============================================================================#
#  School & Summer Meals Module – 2021 ME 
#=============================================================================#
# NOTE FOR FUTURE USE: 
...
sub schoolsummermeals
{
	...
	if ($in->{'child_number'} == 0) {
		...
		if ($in->{'nsbp'}==1 || $in->{'frpl'} == 1 || $in->{'fsmp'} == 1) {
			...
			$elem_cep_lunch = $in->{'cep'}; 
			$elem_cep_bkfst = $in->{'cep'}; 
			$ms_cep_lunch = $in->{'cep'}; 
			$ms_cep_bkfst = $in->{'cep'};  
			$hs_cep_lunch = $in->{'cep'}; 
			$hs_cep_bkfst = $in->{'cep'};  
		...
		for (my $i = 1; $i <= 5; $i++) {
			...
			if (($in->{'frpl'}==1 && $in->{'child'.$i.'_age'} >= 5) || ($in->{'prek'} == 1 && $in->{'child'.$i.'_age'} >= $out->{'prek_age_min'} && $in->{'child'.$i.'_age'} <= $out->{'prek_age_max'})) { #Removing from code, not relevant to NH since it has no state Pre-K program:  (($in->{'prek'} == 1 && $in->{'child'.$i.'_age'} >= $prek_age_min && $in->{'child'.$i.'_age'} <= $prek_age_max)|| 
				if($in->{'child'.$i.'_age'} < 11) {
			...
			# Because of the name of DC's program when we coded this in 2017, we are using the variable "fsmp" here. Eventually we could change this to use "sfsp" to correspond to the federal name for the program, but leaving in for now for compatibility.
