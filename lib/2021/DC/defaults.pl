...
sub defaults {
    ...
    @{$self->{'order'}} = qw(interest parent_earnings filing_status unemployment transportation ssp ssi fed_hlth_insurance hlth child_support child_care ccdf tanf child_care ccdf afterschool tanf sec8 fsp_assets fsp liheap fsp schoolsummermeals wic fedtax eitc payroll ctc statetax food lifeline salestax other);
	
	
	#Loops (FROM PA, MAY CHANGE FOR DC):
	...
	#We calculate fsp before LIHEAP but LIHEAP can also reduce shelter costs, which are incorporated into SNAP/fsp. So we run SNAP first, using a potentially higher shelter cost (and shelter deduction) than the family actually gets. We then calculate LIHEAP, for which families on SNAP are categorically eligible, to potentially reduce sheleter costs. We then run SNAP again with the more accurate shelter cost, reduced due to LIHEAP.
	
	#Below is an explanation of the "order" above for New Hampshire, provided here as an example of the methodological concerns used for determining the sequence of codes. This may be different for other jurisdictions, but the order above ('order') arranges perl files in a sequence such that each subroutine/function has the inputs it needs from previous programs, including proxies or zeroed-out variables established in earlier run programs that are redefined and used in loops by later programs. Please note that there are some repeated codes/functions, also explained below. 
